...
entrezids = fData(vanDeVijver)$EntrezGene.ID
valid_indices = which(!is.na(entrezids), arr.ind = TRUE)
entrezids_valid = as.integer(entrezids[valid_indices])
vanDeVijver_valid = vanDeVijver[valid_indices,]
exprs_valid = exprs(vanDeVijver_valid)
write(entrezids_valid, file="EntrezGeneIds-van.csv",sep="\n")
write.table(exprs_valid, file="vanDeVijver.csv", sep="," )
